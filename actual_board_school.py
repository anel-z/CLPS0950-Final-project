...
words_for_board_school = random.sample(word_list_school, 8)
print('word bank')
print(words_for_board_school)


individual_letters = []

for word in words_for_board_school:
    letter_list = []
    for letter in word:
...
n=0
letter_positions = {w : [] for w in words_for_board_school}
while n<len(words_for_board_school):
  possible_directions = []
  print(words_for_board_school[n])
  while possible_directions == []:
    len_word = len(words_for_board_school[n])
  ...
  if direction[0] == 'horizontal left':
    ...
    for x in range (0, (len_word)):
      ...
      letter_positions[words_for_board_school[n]].append(row_index * 15 + (column_index - x))
    ...
    for x in range (0, len_word):
      ...
      letter_positions[words_for_board_school[n]].append((row_index+x) * 15 + (column_index-x))
