...
words_for_board_sports = random.sample(word_list_sports, 8)
print('word bank')
print(words_for_board_sports)


individual_letters = []

for word in words_for_board_sports:
    letter_list = []
    for letter in word:
...
n=0
letter_positions = {w : [] for w in words_for_board_sports}
while n<len(words_for_board_sports):
  possible_directions = []
  print(words_for_board_sports[n])
  while possible_directions == []:
    len_word = len(words_for_board_sports[n])
  ...
  if direction[0] == 'horizontal left':
    ...
    for x in range (0, (len_word)):
      ...
      letter_positions[words_for_board_sports[n]].append(row_index * 15 + (column_index - x))
    ...
    for x in range (0, (len_word)):
      ...
      letter_positions[words_for_board_sports[n]].append((row_index-x) * 15 + column_index)
    ...
    for x in range (0, len_word):
      ...
      letter_positions[words_for_board_sports[n]].append((row_index+x) * 15 + (column_index-x))
